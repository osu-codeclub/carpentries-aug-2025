...
# Challenge 2
# Use mutate() to create a new column called gdp_billion that has the absolute
# GDP (i.e., not relative to population size) and does so in units of billions
# (i.e., 2.3 would mean 2.3 billion).

# (Bonus) Use mutate() to create a new column planet that has the value earth
# in every row.
...
Therefore, you'll only modify existing data (e.g., delete rows)
when you assign the output back to the input object,
...
<details><summary>{{< fa user-edit >}} How many rows did we get?</summary>
<p>
We got 21 rows, as shown in the first line of the output: `21 x 6` (rows x columns)
</p>
</details>

As we could see above, `filter()` **outputs rows that satisfy the condition(s)**
you specify. These conditions don't have to be based on numeric comparisons:
...
By default, multiple conditions are combined in a Boolean ***AND*** fashion.
...
If you want to combine conditions in a Boolean ***OR*** fashion,
...
Finally, let's practice once more with "pipelines" that use multiple _dplyr_ functions:
we'll `filter` rows, then `select` columns, and finally `rename` one of the
remaining columns:
...
Sorting can help you find the observations with the smallest or largest values
for a certain column: above, we see that the the smallest population size in
...
_(Bonus)_ Use `mutate()` to create a new column `planet` that has the value
`earth` in every row.
