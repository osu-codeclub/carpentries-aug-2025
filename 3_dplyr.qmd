...
The [_dplyr_](https://cran.r-project.org/package=dplyr) package provides highly
useful functions for **manipulating data frames** or "data wrangling".
...
With "dataframe-centric", I mean that we try to do most if not all data analysis
while keeping the data in the dataframe data structure.
...
- `country` (stored as a `factor`, an alternative to `character` that can be used
  for categorical data -- see [here](./2_data-structures.qmd#factors))
...
You can also **specify columns that should be removed**,
by prefacing their name with a `!` (or a `-`):

```{r}
# This will keep all columns _except_ continent:
...
But if that sounds scary, recall that all _dplyr_ functions take a data frame
as the input, and output a ***new***, modified data frame.
...
In this first `filter()` example,
we'll keep observations (remember, these are countries in a given year)
...
So, `filter()` will **keep rows that satisfy the condition(s)** you specify.
...
For example, to calculate the mean GPD and life expectancy across the entire
dataset:
...
For example, to get the mean GDP and mean life expectancy separately
for each continent:
